...
EIAModule.prototype.categoryRequest = function(request, callback) {
    ...
    var categoryURL =
        "https://api.eia.gov/category/?api_key={apiKey}&category_id={category}";
    ...
    CitySDK.prototype.sdkInstance.ajaxRequest(categoryURL).done(function(
        response) {
...
EIAModule.prototype.seriesRequest = function(request, callback) {
    ...
    var seriesURL =
        "https://api.eia.gov/series/?api_key={apiKey}&series_id={series}"
    ...
    CitySDK.prototype.sdkInstance.ajaxRequest(seriesURL).done(function(
        response) {
        response = $.parseJSON(response);
        callback(response);
    });
